...
final class RealisticModelTests: XCTestCase {
  ...
  func testRealModelDownloadAndInference() async throws {
      ...
      AppLogger.shared.info(
        "RealisticModelTests", "Skipping real model test in simulator - MLX not available")
      throw XCTSkip("Skipping real model test in simulator - MLX not available")
    #endif
    let shouldRunRealTests =
      ProcessInfo.processInfo.environment["MLXENGINE_RUN_REAL_TESTS"] == "true"
    if !shouldRunRealTests {
      AppLogger.shared.info(
        "RealisticModelTests",
        "Skipping real model test - set MLXENGINE_RUN_REAL_TESTS=true to enable")
    ...
    AppLogger.shared.info(
      "RealisticModelTests", "Selected model",
      context: ["model": testModel.name, "hubId": testModel.hubId])
    let downloadStartTime = Date()
    let downloader = ModelDownloader()
    let downloadProgressCollector = ProgressCollector()
    AppLogger.shared.info(
      "RealisticModelTests", "Initiating model download",
      context: ["startTime": "\(downloadStartTime)"])
    let modelPath = try await downloader.downloadModel(testModel) { progress in
      AppLogger.shared.info(
        "RealisticModelTests", "Download progress",
        context: ["progress": "\(Int(progress * 100))%", "time": "\(Date())"])
      Task { await downloadProgressCollector.addProgress(progress) }
    }
    let downloadEndTime = Date()
    AppLogger.shared.info(
      "RealisticModelTests", "Model download complete",
      context: ["elapsed": "\(downloadEndTime.timeIntervalSince(downloadStartTime))s"])
    let downloadProgress = await downloadProgressCollector.getProgressValues()
    AppLogger.shared.info(
      "RealisticModelTests", "Download completed",
      context: ["modelPath": modelPath.path, "progressPoints": "\(downloadProgress.count)"])
    ...
    for file in modelFiles {
      ...
      {
        AppLogger.shared.info(
          "RealisticModelTests", "Model file size",
          context: ["file": file, "size": "\(size)", "time": "\(Date())"])
        totalModelSize += size
      } else {
        AppLogger.shared.warning(
          "RealisticModelTests", "Model file not found or unreadable",
          context: ["file": file, "time": "\(Date())"])
      }
    }
    AppLogger.shared.info(
      "RealisticModelTests", "Total model size",
      context: [
        "bytes": "\(totalModelSize)",
        "MB": "\(String(format: "%.2f", Double(totalModelSize) / 1024 / 1024))",
      ])
    ...
    AppLogger.shared.info(
      "RealisticModelTests", "Verifying model files", context: ["time": "\(Date())"])
    ...
    AppLogger.shared.info(
      "RealisticModelTests", "All model files verified", context: ["time": "\(Date())"])
    AppLogger.shared.info(
      "RealisticModelTests", "Testing model loading", context: ["time": "\(Date())"])
    let loadStartTime = Date()
    let loadProgressCollector = ProgressCollector()
    do {
      let engine = try await InferenceEngine.loadModel(testModel) { progress in
        AppLogger.shared.info(
          "RealisticModelTests", "Model load progress",
          context: ["progress": "\(Int(progress * 100))%", "time": "\(Date())"])
        Task { await loadProgressCollector.addProgress(progress) }
      }
      let loadEndTime = Date()
      AppLogger.shared.info(
        "RealisticModelTests", "Model loaded",
        context: ["elapsed": "\(loadEndTime.timeIntervalSince(loadStartTime))s"])
      ...
      for (index, prompt) in testPrompts.enumerated() {
        ...
        let response = try await engine.generate(
          prompt, params: GenerateParams(maxTokens: 50, temperature: 0.7))
        ...
        #if arch(arm64)
          XCTAssertFalse(
            response.contains("[Mock"), "Should not be a mock response on Apple Silicon")
        #endif
        // Check if it's a real MLX response (not mock)
        if response.contains("[Mock") || response.contains("mock") {
          print(
            "⚠️ [REAL MODEL TEST] Using mock response - MLX may not be fully available at \(Date())")
        } else {
      ...
      AppLogger.shared.error(
        "RealisticModelTests", "Model loading failed",
        context: ["elapsed": "\(loadTime)s", "error": error.localizedDescription])
      print(
        "⚠️ [REAL MODEL TEST] Model loading failed after \(String(format: "%.2f", loadTime))s at \(loadEndTime)"
      )
      print("   - Error: \(error.localizedDescription)")
      // Check if it's a known MLX runtime issue
      let errorString = error.localizedDescription.lowercased()
      if errorString.contains("metal") || errorString.contains("steel_attention")
        || errorString.contains("library not found") || errorString.contains("mlx runtime")
        || errorString.contains("file not found")
      {
        print(
          "✅ [REAL MODEL TEST] Expected MLX runtime error - this is normal in test environments at \(Date())"
        )
    ...
    AppLogger.shared.info(
      "RealisticModelTests", "Real model test finished",
      context: ["elapsed": "\(Date().timeIntervalSince(testStartTime))s"])
  }
