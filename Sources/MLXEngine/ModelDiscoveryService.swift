...
public struct ModelDiscoveryService {
    ...
    public static func searchCompatibleMLXModels(query: String, ramGB: Double, platform: String, limit: Int = 20) async throws -> [ModelSummary] {
        let api = HuggingFaceAPI.shared
        let models = try await api.searchModels(query: query, limit: limit)
        let filtered = models.filter { $0.hasMLXFiles() }
    ...
    public static func recommendedMLXModelsForCurrentDevice(limit: Int = 3) async throws -> [ModelSummary] {
        ...
        #endif
        let all = try await searchCompatibleMLXModels(query: "mlx", ramGB: memoryGB, platform: platform, limit: 50)
