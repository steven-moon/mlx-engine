...
public actor ModelDownloader {
  ...
  public func searchModels(query: String, limit: Int = 20) async throws -> [ModelConfiguration] {
    ...
    AppLogger.shared.info(
      "MLXEngine", "🔍 Found \(huggingFaceModels.count) models from Hugging Face search")
    ...
    let filteredModels =
      ...
      .filter { model in
        ...
        if !isMLXCompatible {
          AppLogger.shared.info(
            "MLXEngine",
            "❌ Filtered out: \(model.id) (tags: \(model.tags?.joined(separator: ", ") ?? "none"))")
        }
  ...
  public func downloadModel(
    ...
    AppLogger.shared.info(
      "MLXEngine", "🚀 Using optimized downloader for faster downloads",
    ...
    if let optimizedDownloader = optimizedDownloader {
      return try await optimizedDownloader.downloadModelWithResume(
        ...
        progress: { prog in
          AppLogger.shared.info(
            "MLXEngine", "[Progress] Downloading model...",
            context: ["progress": String(format: "%.2f", prog), "correlationId": correlationId])
          progress(prog)
        })
    } else {
      AppLogger.shared.warning(
        "MLXEngine", "⚠️ Using fallback downloader (optimized downloader not available)",
        context: ["correlationId": correlationId])
      return try await downloadModelFallback(
        config,
        progress: { prog in
          AppLogger.shared.info(
            "MLXEngine", "[Progress] Downloading model (fallback)...",
            context: ["progress": String(format: "%.2f", prog), "correlationId": correlationId])
          progress(prog)
        })
    }
  ...
  private func downloadModelFallback(
      ...
      FileManager.default.fileExists(atPath: safetensorsPath)
    {
      do {
        ...
        #endif
        AppLogger.shared.info(
          "ModelDownloader", "Patched: Linked model.safetensors to main.mlx for MLX compatibility",
          context: ["modelDir": modelDirectory.path])
      } catch {
        AppLogger.shared.warning(
          "ModelDownloader", "Failed to patch main.mlx symlink/copy: \(error)",
          context: ["modelDir": modelDirectory.path])
      }
    }
