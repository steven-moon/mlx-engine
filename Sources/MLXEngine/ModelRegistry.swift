...
public struct ModelRegistry {
  ...
  public static func searchModels(criteria: SearchCriteria) -> [ModelConfiguration] {
    ...
    return allModels.filter { model in
      ...
      if let query = criteria.query, !query.isEmpty {
        let searchText = "\(model.name) \(model.description) \(model.architecture ?? "")"
          .lowercased()
  ...
  public static func getBestModel(
    ...
    let sizeFiltered =
      maxSizeGB != nil ? allModels.filter { $0.estimatedSizeGB ?? 0 <= maxSizeGB! } : allModels
  ...
  private static func isModelOfType(_ model: ModelConfiguration, type: ModelType) -> Bool {
    switch type {
    case .llm:
      return !isModelOfType(model, type: .vlm) && !isModelOfType(model, type: .embedder)
  ...
  public static func findModels(parameterRange: ClosedRange<Double>) -> [ModelConfiguration] {
    allModels.filter { model in
      ...
      if params.contains("0.5b") {
        paramValue = 0.5
      } else if params.contains("1b") {
        paramValue = 1.0
      } else if params.contains("1.5b") {
        paramValue = 1.5
      } else if params.contains("2b") {
        paramValue = 2.0
      } else if params.contains("3b") {
        paramValue = 3.0
      } else if params.contains("7b") {
        paramValue = 7.0
      } else if params.contains("8b") {
        paramValue = 8.0
      } else if params.contains("13b") {
        paramValue = 13.0
      } else if params.contains("30b") {
        paramValue = 30.0
      } else {
        return false
      }
  ...
  public static func isModelSupported(_ model: ModelConfiguration, ramGB: Double, platform: String)
    -> Bool
  {
